...
I choosen to build an application that could report humidity and temperature from garden community close to Brommaplan
in Stockholm Sweden.
...
changes over time and and there is temperatures at the level of Frost, ie.t  temperaturegoes under 0 degrees 
Celius during spring.
...
provides functionality to support the ***main** function.
 ...
 - temphum.py, functionality to read temperature and humidity
 - wifi.py, functionality to connect to a WIFI network. Source code snatched and rewritten.
 - lora.py, functionality to connect to a LoRa network. Source code copied and refactored to suit my means
 - http_requests.py, functionality to post data to **DataCake**
 - credentintials to WIFI and Datacake.   
````
file structure
.
├── LICENSE
...
The are a setup phase, setting up WIFI, LoRA and sensors. and than a runtime phase where sensors are read
and posted to DataCake or some similar system. The intension is to have as much details to the the main function
